...
![设置 → 模型 → 模型目录：点模型行右侧的 > 箭头展开](docs/models-image-input.png)

展开后，「上下文窗口 / 最大输出 token」两个输入框下方就是「**图片输入**」勾选框：

1. 「设置 → 模型」，选中要改的路由（如上图的自定义路由）；
2. 「模型目录」里找到目标模型，点它右侧的 `>` 箭头展开；
3. 勾上「**图片输入**」（= 声明该模型 `input: ["text","image"]`）；
