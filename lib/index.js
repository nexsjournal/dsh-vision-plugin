...
function apply(ctx, rowConfig) {
   ...
   * Resolved per call so card / vision_configure edits apply live.
  ...
  tools.register(
    defineTool({
      ...
      description:
        "配置 dsh-vision 视觉端点（保存即热生效，无需重启、无需配置卡片）。baseURL / model / apiKey 均可选，传哪一项就更新哪一项（不传或留空=不改）。三项存入 DSH 凭据服务。注意：apiKey 会出现在本次对话并写入凭据库，个人/本地 DSH 可用；共享部署建议改用环境变量 DSH_VISION_API_KEY。配置后建议跑 vision_test 验证连接。",
      parameters: {
