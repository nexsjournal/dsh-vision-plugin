 ...
 * Settings -> Plugins card can set them live (hot, no restart):
...
// Named credential refs (managed by the Settings -> Plugins card).
...
function apply(ctx, rowConfig) {
  ...
  /**
   * Resolve one credential ref: credentials value, else same-named env var,
   * else the row-config fallback. Resolved per call so card edits apply live.
   */
  async function resolveRef(ref, rowFallback) {
    ...
    try {
      if (credentials !== undefined) {
        ...
        if (r && typeof r.value === "string" && r.value.trim()) v = r.value.trim();
      }
    } catch (e) {
      /* fall through to env */
    }
    if (!v) {
      ...
      if (typeof envVal === "string" && envVal.trim()) v = envVal.trim();
    }
    if (!v && rowFallback) v = rowFallback;
    return v;
  }

  /** The live endpoint (baseURL + model) from credentials — resolved per call. */
  ...
  /** Resolve the API key: credentials under REF_KEY, else env var. */
  function resolveApiKey() {
    return resolveRef(REF_KEY, "");
  }
  ...
  class VisionAdapter extends LlmAdapter {
    ...
    async *stream(options) {
      ...
      if (!baseURL) throw new Error("未配置视觉模型 Base URL：在「设置 → 插件 → 视觉识别」卡片填写，或设置环境变量 " + REF_BASE);
      if (!model) throw new Error("未配置视觉模型 model：在「设置 → 插件 → 视觉识别」卡片填写，或设置环境变量 " + REF_MODEL);
      const signal = options.signal;
      const apiKey = await resolveApiKey();
      if (!apiKey) throw new Error("未配置视觉模型 API Key：在「设置 → 插件 → 视觉识别」卡片填写，或设置环境变量 " + REF_KEY);
      const mapped = [];
  ...
  tools.register(
    defineTool({
      name: "vision_status",
      description:
        "查看 dsh-vision 视觉插件状态：第三方视觉模型 provider/model 是否注册成功、Base URL / 模型 / API Key 是否就绪。用户询问识图能力、图片识别失败原因或视觉模型配置时，先调用本工具。",
      parameters: {},
      output: { schema: { type: "json" }, render: renderJson },
      async execute() {
        const s = staticCfg();
        const ep = await endpoint().catch(() => ({ baseURL: "", model: "" }));
        const key = await resolveApiKey().catch(() => "");
        const ready = ep.baseURL && ep.model && key;
        return {
          ok: true,
          enabled: s.enabled,
          ...
          configured: { baseURL: ep.baseURL.length > 0, model: ep.model.length > 0, apiKey: key.length > 0 },
          hint: adapterOk
            ? ready
              ? "已就绪：在会话模型选择器中切换到 " + providerId + " 的模型后，直接发送图片即可由第三方视觉模型识别。"
              : "请先到「设置 → 插件 → 视觉识别」卡片填写 Base URL / 模型 / API Key（凭据 " + REF_BASE + " / " + REF_MODEL + " / " + REF_KEY + "）。"
    ...
    defineTool({
      ...
      async execute(args) {
        ...
        try {
          ...
          if (!ep.baseURL || !ep.model) return { ok: false, error: "未配置 Base URL / model（凭据 " + REF_BASE + " / " + REF_MODEL + "，或「设置 → 插件 → 视觉识别」卡片）" };
